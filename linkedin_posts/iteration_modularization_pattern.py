...
# Iteration and modularization patterns
def chunk_data(data: List[Any], chunk_size: int) -> Iterator[List[Any]]:
    """Split data into chunks for batch processing"""
    for i in range(0, len(data), chunk_size):
        yield data[i:i + chunk_size]

def process_targets_in_batches(targets: List[ScanTarget], batch_size: int = 10) -> Iterator[List[ScanTarget]]:
    """Process targets in batches to avoid memory issues"""
    for batch in chunk_data(targets, batch_size):
        yield batch

def group_results_by_security_level(results: List[ScanResult]) -> Dict[SecurityLevel, List[ScanResult]]:
    """Group scan results by security level"""
    grouped = defaultdict(list)
    for result in results:
        grouped[result.security_level].append(result)
    return dict(grouped)

def aggregate_vulnerabilities(results: List[ScanResult]) -> Dict[str, int]:
    """Aggregate vulnerabilities across all results"""
    all_vulnerabilities = chain.from_iterable(
        result.vulnerabilities for result in results
    )
    return Counter(all_vulnerabilities)

def map_targets_to_results(targets: List[ScanTarget], results: List[ScanResult]) -> Dict[str, List[ScanResult]]:
    """Map targets to their corresponding results"""
    target_results = defaultdict(list)
    for result in results:
        target_results[result.target].append(result)
    return dict(target_results)

# Modular processing functions
def process_port_scan(target: ScanTarget) -> ScanResult:
    """Process a single port scan"""
    # Simulate port scan logic
    return ScanResult(
        target=target.host,
...
def process_vulnerability_scan(target: ScanTarget) -> ScanResult:
    """Process a single vulnerability scan"""
    # Simulate vulnerability scan logic
    return ScanResult(
        target=target.host,
        port=target.ports[0] if target.ports else None,
        is_open=True,
...
def process_network_scan(target: ScanTarget) -> ScanResult:
    """Process a single network scan"""
    # Simulate network scan logic
    return ScanResult(
        target=target.host,
        port=target.ports[0] if target.ports else None,
        is_open=True,
...
# Scan type processor mapping
SCAN_PROCESSORS = {
    ScanType.PORT: process_port_scan,
    ScanType.VULNERABILITY: process_vulnerability_scan,
    ScanType.NETWORK: process_network_scan,
    ScanType.WEB: process_vulnerability_scan  # Web scans use vulnerability processor
}

def get_scan_processor(scan_type: ScanType) -> Callable[[ScanTarget], ScanResult]:
    """Get the appropriate scan processor for the scan type"""
    return SCAN_PROCESSORS.get(scan_type, process_port_scan)
...
self.config = config
    ...
    async def scan_target(self, target: ScanTarget) -> ScanResult:
        ...
        processor = get_scan_processor(target.scan_type)
    ...
    async def scan_multiple_targets(self, targets: List[ScanTarget]) -> List[ScanResult]:
        """Scan multiple targets concurrently using iteration patterns"""
        # Process targets in batches to avoid overwhelming the system
        batch_size = self.config.get('batch_size', 10)
        all_results = []
        
        for batch in process_targets_in_batches(targets, batch_size):
            batch_results = await map_async(self.scan_target, batch)
...
async async def handle_scan_request(request_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    config = request_data.get('config', {'max_concurrent_scans': 10, 'batch_size': 5})
...
# Example usage
async def main():
    """Example usage of functional security scanner with iteration patterns"""
    # Sample scan request
    request_data = {
        'targets': [
            {
