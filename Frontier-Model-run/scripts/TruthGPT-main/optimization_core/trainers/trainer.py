...
class GenericTrainer:
    def __init__(
        ...
        self.model = AutoModelForCausalLM.from_pretrained(cfg.model_name, torch_dtype=load_dtype)
                ...
                for step, batch in enumerate(self.train_loader, start=1):
                    try:
                        ...
                        if not torch.isfinite(loss):
                            logger.warning(f"Skipping step {step} due to non-finite loss: {loss.item()}")
                        ...
                        if step % self.cfg.grad_accum_steps == 0:
                            ...
                            # Get model for gradient clipping (handle DataParallel)
                            model_for_clipping = self.model.module if self._is_parallel else self.model
