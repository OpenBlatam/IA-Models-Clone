...
from unified_core.auth.users import current_user_with_expired_token
from unified_core.configs.app_configs import APP_API_PREFIX
from unified_core.server.onyx_api.ingestion import api_key_dep
...
def check_router_auth(
    ...
    control_plane_dep = fetch_ee_implementation_or_noop(
        "unified_core.server.tenants.access", "control_plane_dep"
    )
    current_cloud_superuser = fetch_ee_implementation_or_noop(
        "unified_core.auth.users", "current_cloud_superuser"
    )
