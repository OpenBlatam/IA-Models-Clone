...
from unified_core.agents.agent_search.dr.sub_agents.image_generation.models import (
    GeneratedImageFullResult,
)
from unified_core.auth.schemas import UserRole
from unified_core.configs.chat_configs import NUM_POSTPROCESSED_RESULTS
from unified_core.configs.constants import (
    DEFAULT_BOOST,
...
from unified_core.db.enums import (
    AccessType,
    EmbeddingPrecision,
    IndexingMode,
    SyncType,
...
from unified_core.configs.constants import NotificationType
from unified_core.configs.constants import SearchFeedbackType
from unified_core.configs.constants import TokenRateLimitScope
from unified_core.connectors.models import InputType
from unified_core.db.enums import ChatSessionSharedStatus
...
from unified_core.db.enums import PermissionSyncStatus
from unified_core.db.enums import TaskStatus
from unified_core.db.pydantic_type import PydanticListType, PydanticType
from unified_core.kg.models import KGEntityTypeAttributes
...
from unified_core.llm.override_models import LLMOverride
from unified_core.llm.override_models import PromptOverride
from unified_core.context.search.enums import RecencyBiasSetting
from unified_core.kg.models import KGStage
from unified_core.server.features.mcp.models import MCPConnectionData
from unified_core.utils.encryption import decrypt_bytes_to_string
from unified_core.utils.encryption import encrypt_string_to_bytes
from unified_core.utils.headers import HeaderItemDict
...
class User(SQLAlchemyBaseUserTableUUID, Base):
    ...
    continuous_agents: Mapped[list["ContinuousAgent"]] = relationship(
        "ContinuousAgent",
        back_populates="user",
        cascade="all, delete-orphan",
    )
...
class ContinuousAgent(Base):
    """
    Represents a continuous agent that runs 24/7 on the server.
    Agents execute tasks periodically even when the user's computer is off.
    """
    ...
    stats: Mapped[dict[str, Any]] = mapped_column(
        postgresql.JSONB(), nullable=False, default=lambda: {
            "total_executions": 0,
    ...
    # Relationships
    user: Mapped["User"] = relationship("User", back_populates="continuous_agents")
    execution_logs: Mapped[list["ContinuousAgentExecutionLog"]] = relationship(
    ...
    __table_args__ = (
        Index("ix_continuous_agent_user_active", "user_id", "is_active"),
        Index("ix_continuous_agent_active", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<ContinuousAgent(id={self.id!r}, name={self.name!r}, is_active={self.is_active!r})>"


class ContinuousAgentExecutionLog(Base):
    """
    Logs execution history for continuous agents.
    Tracks each execution attempt, success/failure, and resource usage.
    """

    __tablename__ = "continuous_agent_execution_log"

    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)

    agent_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("continuous_agent.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    # Relationships
    agent: Mapped["ContinuousAgent"] = relationship(
        "ContinuousAgent", back_populates="execution_logs"
    )

    __table_args__ = (
        Index("ix_agent_execution_log_agent_time", "agent_id", desc("started_at")),
        Index("ix_agent_execution_log_status", "status"),
    )

    def __repr__(self) -> str:
        return (
            f"<ContinuousAgentExecutionLog(id={self.id!r}, "
