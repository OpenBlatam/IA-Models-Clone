...
class LLMService:
    ...
    def __init__(
        ...
        self.deepseek_base_url = getattr(settings, 'DEEPSEEK_API_BASE_URL', self.DEEPSEEK_API_BASE)

        if not self.api_key and not self.deepseek_api_key:
            logger.warning("Ni OpenRouter ni DeepSeek API keys configuradas. LLM fallará.")
        ...
        # Cliente HTTP async base, se sobreescribirá por request si es necesario
    ...
    async def _process_stream(self, request: LLMRequest) -> AsyncIterator[str]:
        ...
        async with self.semaphore:
            try:
                ...
                base_url, headers = self._get_request_config(request.model)
    ...
    def _get_request_config(self, model: str) -> tuple[str, dict]:
        """Obtener configuración de request (base_url, headers) según el modelo."""
        # Usar API nativa de DeepSeek si es un modelo deepseek y tenemos la key
        ...
        if not self.api_key:
            raise TaskProcessingError(f"No hay API key configurada para el modelo {model}")
    ...
    async def _make_request(self, request: LLMRequest) -> LLMResponse:
        """Hacer request a la API (OpenRouter o DeepSeek)."""
        async with self.semaphore:
            start_time = datetime.now()
