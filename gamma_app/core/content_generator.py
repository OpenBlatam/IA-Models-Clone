...
class ContentGenerator:
    ...
    async def close(self):
        """Close content generator and clean up resources"""
        try:
            if self.openai_client:
                # OpenAI client doesn't need explicit closing, but we can clear it
                self.openai_client = None
            if self.anthropic_client:
                # Anthropic client doesn't need explicit closing, but we can clear it
                self.anthropic_client = None
            # Clear local models to free memory
            if self.local_models:
                self.local_models.clear()
            logger.info("Content generator closed successfully")
        except Exception as e:
