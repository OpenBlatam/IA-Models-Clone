...
async def rate_limited_operation(
    ...
    Usage:
        async with rate_limited_operation(rate_limiter, user_id="user123"):
            # operation code
    """
    await rate_limiter.wait(user_id)
    try:
        yield
    finally:
...
async def transaction_operation(
    ...
    Usage:
        async with transaction_operation(transaction_manager, "update_task"):
            # transaction code
    """
    transaction = await transaction_manager.begin(operation_name)
    try:
        yield transaction
        await transaction_manager.commit(transaction)
    except Exception as e:
        await transaction_manager.rollback(transaction)
        raise
