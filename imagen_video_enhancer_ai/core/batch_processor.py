"""
Batch Processor for Imagen Video Enhancer AI
=============================================
...
class BatchProcessor:
    ...
    async def _process_item(
        ...
        async with self._semaphore:
            try:
                # Process item using the provided function
                item_result = await process_func(item)
                
                result.results.append({
                    "file_path": item.file_path,
                    "service_type": item.service_type,
                    "success": True,
                    "task_id": item_result if isinstance(item_result, str) else item_result.get("task_id"),
