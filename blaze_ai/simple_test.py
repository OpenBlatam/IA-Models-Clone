...
Improved version with better error handling and reporting.
"""
...
def test_disk_space() -> Tuple[bool, str, float]:
    ...
    try:
        ...
        if free_gb < 1.0:
            print(f"  ⚠️  Low disk space warning: {free_gb:.2f} GB available")
...
"""
        ...
        exec(test_code)
        print("  ✅ Basic Python syntax test passed")
...
def run_all_tests() -> bool:
    ...
    if summary['success_rate'] == 100:
        ...
        print(f"\n⚠️  MOST TESTS PASSED ({summary['success_rate']:.1f}%). Some issues need attention.")
        return True
    else:
        print(f"\n💥 MANY TESTS FAILED ({summary['success_rate']:.1f}%). System needs significant fixes.")
        return False
