...
import psutil
import gc
from ..utils.robust_helpers import RobustRetry, CircuitBreaker, RateLimiter, validate_input
...
class ContinuousGenerationEngine:
    ...
    def __init__(self, config: BulkAIConfig):
        self.config = config
        self.truthgpt_engine = None  # Will be set by BulkAISystem
        ...
        self.storage_service = None  # Will be set for persistence
    ...
    async def _generate_document(self, task: GenerationTask) -> Optional[Dict[str, Any]]:
        """Generate a single document with robust error handling."""
        # Check rate limit
        if not await self.rate_limiter.acquire():
            logger.warning("Rate limit exceeded, waiting...")
            await self.rate_limiter.wait()
        
        # Use truthgpt_engine from ContinuousGenerationEngine if available
        truthgpt_engine = self.truthgpt_engine or getattr(self.generation_engine, 'truthgpt_engine', None)
        ...
        async def _generate_with_circuit():
            """Inner generation function."""
            # Use real TruthGPT Engine if available
            if truthgpt_engine:
                from ..core.truthgpt_engine import GenerationContext
                from ..models.schemas import GenerationConfig
                
                # Create generation config from task config
                ...
                result = await truthgpt_engine.generate_document(context)
                
                # Format result to match expected structure
                document = {
                    "id": task.task_id,
                    "query": task.query,
                    "content": result.get("content", {}).get("content", "") if isinstance(result.get("content"), dict) else result.get("content", ""),
                    "timestamp": result.get("metadata", {}).get("created_at", datetime.now().isoformat()),
                    "model_used": gen_config.model,
                    "generation_time": result.get("metadata", {}).get("generation_time", 0),
                    "quality_score": result.get("analysis", {}).get("quality_score", 0.7),
                ...
                logger.warning("TruthGPT Engine not available, using fallback generation")
                document = {
                    "id": task.task_id,
                    "query": task.query,
        ...
        try:
            ...
            async def _wrapped_generate():
                return await self.circuit_breaker.call_async(_generate_with_circuit)
            
            return await retry.execute(_wrapped_generate)
        except Exception as e:
            self.error_count += 1
            logger.error(f"Document generation failed after all retries: {e}")
            return None
...
class BulkAISystem:
    ...
    def __init__(self, config: BulkAIConfig, truthgpt_engine=None):
        self.config = config
        self.truthgpt_engine = truthgpt_engine  # TruthGPT Engine for real generation
        self.model_selector = AdaptiveModelSelector(config)
        self.generation_engine = ContinuousGenerationEngine(config)
        self.generation_engine.truthgpt_engine = truthgpt_engine  # Pass engine to generator
        ...
        self.rate_limiter = RateLimiter(rate=10.0, capacity=50.0)  # 10 req/sec, burst 50
