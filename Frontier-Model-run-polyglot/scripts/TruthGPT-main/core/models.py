...
class ModelConfig:
    ...
    def __post_init__(self):
        """Set device automatically if not specified and validate model_type"""
        # Validate model_type
        if not isinstance(self.model_type, ModelType):
            try:
                # Try to convert string to ModelType
                self.model_type = ModelType(self.model_type)
            except (ValueError, TypeError):
                raise ValueError(f"Invalid model_type: {self.model_type}. Must be one of {[e.value for e in ModelType]}")
...
class ModelManager:
    ...
    def save_model(self, save_path: str, include_optimizer: bool = False, optimizer_state: Optional[Dict] = None) -> None:
        ...
        if self.model is None:
            raise ValueError("No model loaded. Call load_model() first before saving.")
