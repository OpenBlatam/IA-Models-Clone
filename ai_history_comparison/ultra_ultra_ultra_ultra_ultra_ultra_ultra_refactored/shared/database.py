...
Gestor de base de datos compartido entre todos los microservicios.
"""
...
class DatabaseManager:
    """Gestor de base de datos compartido."""
    
    def __init__(self, db_path: str = "ai_history_comparison_ultra.db"):
    ...
    async def _create_tables(self):
        ...
        tables = [
            ...
            CREATE TABLE IF NOT EXISTS comparison_results (
                ...
                comparison_type TEXT DEFAULT 'comprehensive',
            ...
            CREATE TABLE IF NOT EXISTS system_metrics (
                ...
                total_quality_reports INTEGER DEFAULT 0,
                active_jobs INTEGER DEFAULT 0,
                average_quality_score REAL DEFAULT 0.0,
                average_coherence_score REAL DEFAULT 0.0,
                average_relevance_score REAL DEFAULT 0.0,
                average_creativity_score REAL DEFAULT 0.0,
    ...
    async def _create_indexes(self):
        ...
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_history_model_version ON history_entries(model_version)",
            ...
            "CREATE INDEX IF NOT EXISTS idx_comparison_timestamp ON comparison_results(timestamp)",
            "CREATE INDEX IF NOT EXISTS idx_quality_entry_id ON quality_reports(entry_id)",
            "CREATE INDEX IF NOT EXISTS idx_quality_timestamp ON quality_reports(timestamp)",
            "CREATE INDEX IF NOT EXISTS idx_jobs_status ON analysis_jobs(status)",
            "CREATE INDEX IF NOT EXISTS idx_jobs_type ON analysis_jobs(job_type)",
            "CREATE INDEX IF NOT EXISTS idx_jobs_timestamp ON analysis_jobs(timestamp)",
            "CREATE INDEX IF NOT EXISTS idx_trends_start_date ON trend_analyses(start_date)",
            "CREATE INDEX IF NOT EXISTS idx_trends_end_date ON trend_analyses(end_date)",
    ...
    async def update_history_entry(self, entry_id: str, updates: Dict[str, Any]) -> HistoryEntry:
        """Actualizar una entrada de historial."""
        try:
            # Obtener entrada existente
            existing_entry = await self.get_history_entry(entry_id)
            ...
            # Recrear objeto
            updated_entry = HistoryEntry(**updated_data)
            ...
            async with self._lock:
                await self.connection.execute(
                    ...
                    UPDATE history_entries SET
                        model_type = ?, model_version = ?, prompt = ?, response = ?,
                        metadata = ?, response_time_ms = ?, token_count = ?, cost_usd = ?,
                        coherence_score = ?, relevance_score = ?, creativity_score = ?,
                        accuracy_score = ?, analysis_data = ?
                    WHERE id = ?
                    """,
                    (
                        updated_entry.model_type.value,
                        updated_entry.model_version,
                        updated_entry.prompt,
                        updated_entry.response,
                        json.dumps(updated_entry.metadata),
                        updated_entry.response_time_ms,
                        updated_entry.token_count,
                        updated_entry.cost_usd,
                        updated_entry.coherence_score,
                        updated_entry.relevance_score,
                        updated_entry.creativity_score,
                        updated_entry.accuracy_score,
                        json.dumps(updated_entry.analysis_data) if updated_entry.analysis_data else None,
                        entry_id
                    )
                )
                await self.connection.commit()
    ...
    async def save_entry_analysis(self, entry_id: str, analysis: Dict[str, Any]):
        """Guardar análisis de una entrada."""
        try:
            async with self._lock:
                await self.connection.execute(
                    "UPDATE history_entries SET analysis_data = ? WHERE id = ?",
                    (json.dumps(analysis), entry_id)
                )
                await self.connection.commit()
        except Exception as e:
            logger.error(f"Error saving entry analysis {entry_id}: {e}")
            raise
    ...
    def _row_to_history_entry(self, row) -> HistoryEntry:
        ...
        return HistoryEntry(
            ...
            analysis_data=json.loads(row[14]) if row[14] else None
        )
            ...
            async with self._lock:
                await self.connection.execute(
                    ...
                    INSERT INTO comparison_results (
                        ...
                        differences, improvements, analysis_details, comparison_type, confidence_score
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        comparison.id,
            ...
            async with self._lock:
                await self.connection.execute(
                    ...
                    INSERT INTO quality_reports (
                        ...
                        analysis_method, confidence_score, assessor_version
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        report.id,
            ...
            async with self._lock:
                await self.connection.execute(
                    ...
                    INSERT INTO analysis_jobs (
                        ...
                        target_entries, results, error_message, created_by, priority, estimated_duration
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        job.id,
        ...
        try:
            query = """
                SELECT 
                    ...
                    MIN(overall_similarity) as min_similarity,
            ...
            async with self.connection.execute(query, params) as cursor:
                ...
                return {
                    "average_similarity": row[0] or 0.0,
                    "min_similarity": row[1] or 0.0,
                    "max_similarity": row[2] or 0.0,
                    "total_comparisons": row[3] or 0,
                    "period": {
