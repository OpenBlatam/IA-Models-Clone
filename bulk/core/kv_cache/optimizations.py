...
from kv_cache.constants import INT8_MAX_VALUE, INT8_MIN_VALUE
from kv_cache.interfaces import IQuantizer, ICompressor

logger = logging.getLogger(__name__)


class FastQuantizer(IQuantizer):
    ...
    def _quantize_int8(self, key: torch.Tensor, value: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        value_scale = torch.clamp(value_max / INT8_MAX_VALUE, min=1e-8)
        
        key_q = (key / key_scale).round().clamp(INT8_MIN_VALUE, INT8_MAX_VALUE).to(torch.int8)
        value_q = (value / value_scale).round().clamp(INT8_MIN_VALUE, INT8_MAX_VALUE).to(torch.int8)
        
        return key_q, value_q, key_scale, value_scale
    
    def quantize(self, key: torch.Tensor, value: torch.Tensor, dtype: torch.dtype = torch.float16) -> TensorPair:
...
class FastCompressor(ICompressor):
    ...
    def _compress_truncate(self, key: torch.Tensor, value: torch.Tensor, ratio: float) -> TensorPair:
    ...
    def compress(self, key: torch.Tensor, value: torch.Tensor, dtype: torch.dtype = torch.float16) -> TensorPair:
        """Fast compress."""
...
class FastStorage:
    ...
    def __init__(self):
        ...
        self._access_times: AccessTimesDict = {}
        self._access_counts: AccessCountsDict = {}
        self._lock = torch.multiprocessing.Lock() if hasattr(torch.multiprocessing, 'Lock') else None
    
    def get_fast(self, position: int) -> TensorPair | None:
