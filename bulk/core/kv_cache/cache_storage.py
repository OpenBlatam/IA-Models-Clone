...
from kv_cache.types import TensorPair, CacheDict, AccessTimesDict, AccessCountsDict
...
class CacheStorage:
    ...
    def __init__(self, use_ordered_dict: bool = False):
        ...
        if use_ordered_dict:
            self._cache: OrderedDict[int, TensorPair] = OrderedDict()
        else:
    ...
    def get_access_times(self) -> AccessTimesDict:
        """Get copy of access times."""
        with self._lock:
            return dict(self._access_times)
    
    def get_access_counts(self) -> AccessCountsDict:
