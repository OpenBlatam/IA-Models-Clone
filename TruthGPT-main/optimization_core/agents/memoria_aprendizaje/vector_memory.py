...
class VectorMemoryConfig(BaseModel):
    ...
    embedding_model: str = Field(default="BAAI/bge-small-en-v1.5", description="SentenceTransformer model name (SOTA)")
    episodic_collection: str = "openclaw_episodic"
    semantic_collection: str = "openclaw_semantic"
    papers_collection: str = "openclaw_papers"
...
class NumpyVectorProvider:
    """Lightweight in-memory VectorProvider using Numpy and SentenceTransformers."""
    def __init__(self, config: VectorMemoryConfig) -> None:
        import numpy as np
        try:
            ...
            self.enabled = False
            logger.warning("NumpyProvider: sentence-transformers missing.")
        
        self.np = np
        self.collections: Dict[str, Dict[str, Any]] = {}

    def _get_coll(self, name: str) -> Dict[str, Any]:
        if name not in self.collections:
            self.collections[name] = {"ids": [], "docs": [], "metas": [], "vecs": None}
        return self.collections[name]
    ...
    async def add_document(self, collection: str, content: str, metadata: Dict[str, Any], doc_id: str) -> bool:
        ...
        def _add():
            ...
            coll["docs"].append(content)
            coll["metas"].append(metadata)
            if coll["vecs"] is None:
                coll["vecs"] = self.np.array([vec])
            else:
                coll["vecs"] = self.np.vstack([coll["vecs"], vec])
            return True
    ...
    async def query_documents(self, collection: str, query: str, where: Dict[str, Any], top_k: int) -> List[str]:
        ...
        def _query():
            coll = self._get_coll(collection)
            if coll["vecs"] is None or len(coll["docs"]) == 0:
                return []
            
            q_vec = self.model.encode(query, normalize_embeddings=True)
            scores = self.np.dot(coll["vecs"], q_vec)
            
            # Simple metadata filtering (must match all keys in 'where')
            valid_indices = []
            for i, meta in enumerate(coll["metas"]):
                match = True
                for k, v in where.items():
                    if meta.get(k) != v:
                        match = False
                        break
                if match: valid_indices.append(i)
            ...
            top_indices = [i for _, i in valid_scores[:top_k]]
            return [coll["docs"][i] for i in top_indices]
...
class VectorMemory:
    ...
    def __init__(
        ...
        if self.config.provider_type == "chroma":
            try:
                ...
                # Fallback to numpy
                self.provider = NumpyVectorProvider(self.config)
                self.enabled = getattr(self.provider, "enabled", False)
        elif self.config.provider_type == "numpy":
            try:
                self.provider = NumpyVectorProvider(self.config)
                self.enabled = getattr(self.provider, "enabled", False)
            except Exception as e:
                logger.error("VectorMemory: NumpyProvider failed: %s", e)
    ...
    async def add_paper_chunk(
        ...
        """Add a chunk of a scientific paper to the papers collection."""
        mem_id = f"paper_{paper_id}_{uuid.uuid4().hex[:8]}"
        if not self.enabled:
            return MemoryAddResult(memory_id=mem_id, collection=self.config.papers_collection, success=False)

        meta = {"paper_id": paper_id, "type": "paper_chunk"}
        if metadata:
            meta.update(metadata)
        success = await self.provider.add_document(self.config.papers_collection, chunk_text, meta, mem_id)
        return MemoryAddResult(memory_id=mem_id, collection=self.config.papers_collection, success=success)

    async def search_papers(self, query: str, top_k: int = 5) -> SearchResult:
        """Search across all indexed scientific papers."""
        if not self.enabled:
            return SearchResult(query=query, collection=self.config.papers_collection)
        docs = await self.provider.query_documents(self.config.papers_collection, query, {"type": "paper_chunk"}, top_k)
        return SearchResult(query=query, collection=self.config.papers_collection, documents=docs, count=len(docs))
