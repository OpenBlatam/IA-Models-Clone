...
def _build_catalog() -> Dict[str, Dict[str, Any]]:
    ...
    try:
        from ..razonamiento_planificacion.tools import (
            ...
            PythonExecutionTool, SystemBashTool, DirectoryListTool, GlobTool,
        )
        catalog["web_search"] = {
        ...
        catalog["file_ops"] = {
            "description": "Operaciones de archivos y carpetas locales (lectura, escritura, listado, glob)",
            "tools": {
...
class ComposedAgent(BaseAgent):
    ...
    def __init__(
        ...
        # Initialize ReAct loop backend
        from ..razonamiento_planificacion.orchestrator import MultiUserReActAgent
        
        self.system_prompt = (
            f"Eres '{self.name}', un agente de TruthGPT con el rol: {self.role}.\n"
            f"Tus capacidades son: {', '.join(self._capability_keys)}.\n"
            "Analiza las solicitudes del usuario y utiliza tus herramientas paso a paso para resolverlas."
        )
    ...
    async def process(
        ...
        # Fallback: run using ReAct Agent loop
        user_id = (context or {}).get("user_id", "composer_default")
