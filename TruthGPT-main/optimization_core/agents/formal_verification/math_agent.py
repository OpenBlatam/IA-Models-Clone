...
class MathVerificationAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.system_prompt = (
            ...
            "Tienes acceso al sistema de archivos local de la máquina a través de las herramientas "
            "'file_read', 'file_write', 'directory_list', y 'glob_search'. Si el usuario te proporciona "
            "una ruta, un archivo o te pide buscar código o matemáticas en un directorio local, DEBES "
            "utilizar estas herramientas para explorar y leer los archivos locales en lugar de asumir "
            "que estás en un entorno aislado sin acceso a archivos.\n\n"
            "Siempre usa las herramientas para verificar y leer antes de realizar afirmaciones."
        )

        self.react_agent = MultiUserReActAgent(
            config=config or AgentConfig(name=name, llm_engine=llm_engine),
