            ...
            async with httpx.AsyncClient(follow_redirects=True) as client:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        text = ""
                        for page in doc: # Extract full text for RAG
                            text += page.get_text()
                        
                        # --- RAG Indexing ---
                        indexed_msg = ""
                        try:
                            from optimization_core.agents.memoria_aprendizaje.vector_memory import VectorMemory
                            vmem = VectorMemory()
                            if vmem.enabled:
                                chunks = [text[i:i+1000] for i in range(0, len(text), 800)]
                                for chunk in chunks:
                                    await vmem.add_paper_chunk(paper_id, chunk, {"source": "SOTAPaperScraper"})
                                indexed_msg = f" | ✓ RAG Indexed ({len(chunks)} chunks)"
                        except Exception as e:
                            logger.error(f"Failed to index paper into RAG: {e}")
                            indexed_msg = f" | ✗ RAG Index Failed"
                        
                        # Buscar links de GitHub en el texto
                        import re
                        github_links = re.findall(r"github\.com/[a-zA-Z0-9\-_]+/[a-zA-Z0-9\-_]+", text)
                        
                        return f"✓ PDF Descargado: {file_path}{indexed_msg}\nContexto Extraído: {text[:500]}...\nGitHubs Encontrados: {github_links}"
                    except ImportError:
...
class RAGPaperSearchTool(BaseTool):
    """
    Realiza una búsqueda semántica (RAG) en la colección de papers científicos descargados.
    Útil para preguntar sobre detalles técnicos, fórmulas o conceptos de papers SOTA.
    """
    ...
    async def run(self, query: str) -> str:
        try:
            ...
            vmem = VectorMemory()
            if not vmem.enabled:
                return "Error: Vector Memory is not enabled (Missing ChromaDB or sentence-transformers)."
                
            res = await vmem.search_papers(query, top_k=5)
            if not res.documents:
                return f"No se encontró información en los papers descargados para: '{query}'"
                
            out = f"Resultados RAG para '{query}':\n\n"
