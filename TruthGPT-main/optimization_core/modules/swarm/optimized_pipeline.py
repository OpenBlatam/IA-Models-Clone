...
class OptimizedOrchestrator:
    ...
    async def execute_pipeline(self, blueprint: List[str], initial_prompt: str):
        ...
        i = 0
        while i < len(to_run_sequential):
            k = to_run_sequential[i]
            ...
            # Execute math_verifier and system_agent in parallel to reduce wall-clock time
            if k == "math_verifier" and i + 1 < len(to_run_sequential) and to_run_sequential[i+1] == "system_agent":
                logger.info("Executing math_verifier and system_agent in parallel")
                task1 = self.run_phase("math_verifier", current_prompt, context)
                task2 = self.run_phase("system_agent", current_prompt, context)
                
                par_results = await asyncio.gather(task1, task2)
                ...
                current_prompt = f"Previous Context (parallel sync): {par_results[-1]['output'][:600]}\n\nNext Task: {initial_prompt}"
                i += 2
                continue
