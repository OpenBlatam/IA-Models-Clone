...
class PersistenceManager:
    ...
    async def list_all_tasks(self) -> List[TaskSnapshot]:
        """Lists all tasks including completed, running, and paused."""
        async with self.async_session() as session:
            result = await session.execute(
                select(TaskSnapshotModel)
            )
            models = result.scalars().all()
            return [TaskSnapshot.model_validate_json(m.state_json) for m in models]
    ...
    async def update_task_status(self, task_id: str, status: str):
        """Updates a task's status (e.g., 'running', 'paused', 'completed')."""
        async with self.async_session() as session:
            async with session.begin():
                await session.execute(
                    update(TaskSnapshotModel)
