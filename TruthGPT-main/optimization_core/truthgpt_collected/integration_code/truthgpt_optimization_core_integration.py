...
class TruthGPTOptimizationCoreConfig:
    ...
    llm_compiler_config: Dict[str, Any] = field(default_factory=dict)
...
class TruthGPTModel(nn.Module):
    ...
    def __init__(self, config: TruthGPTOptimizationCoreConfig):
        ...
        if config.enable_llm_compiler:
            try:
                import sys
                import os
                sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'papers'))
                from research.paper_llm_compiler import LLMCompilerModule, LLMCompilerConfig
                compiler_cfg = LLMCompilerConfig(
                    hidden_dim=config.hidden_size,
                    **config.llm_compiler_config
                )
                self.llm_compiler_module = LLMCompilerModule(compiler_cfg)
                logger.info("LLM Compiler module integrated (System 5.9)")
            except (ImportError, Exception) as e:
                logger.warning(f"LLM Compiler module not available: {e}")
                self.llm_compiler_module = None
        else:
            self.llm_compiler_module = None
