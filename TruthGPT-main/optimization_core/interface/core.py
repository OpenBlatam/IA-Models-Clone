...
async def handle_personalize():
    ...
    while True:
        ...
        if choice == "0": break
            ...
            USER_PREFS["preferred_engine"] = ",".join(resolved)
        elif choice == "3":
...
def wait_for_user(force: bool = False, timeout: int = 3):
...
_CACHED_BUDGET_STATS = None
_CACHED_BUDGET_MTIME = 0
_LAST_BUDGET_UPDATE = 0

def get_real_budget_stats():
    """Reads actual API budget data from persistence with 2-second cache and file mtime check."""
    ...
    if _CACHED_BUDGET_STATS is not None and mtime == _CACHED_BUDGET_MTIME and (now - _LAST_BUDGET_UPDATE) < 2.0:
...
class TelemetryProvider:
    ...
    _TELEMETRY_UPDATER_STARTED = False
    ...
    @classmethod
    def _start_telemetry_updater(cls):
        import threading
        def updater():
            while True:
                try:
...
def get_claude_header(updates: list[str] = None):
    ...
    user = USER_PREFS.get("user_name", "Explorer")
    engine = USER_PREFS.get("preferred_engine", "deepseek").split(",")[0].strip()
    uptime_s = int(time.time() - START_TIME)
    h, r = divmod(uptime_s, 3600)
    m, s = divmod(r, 60)
    uptime = f"{h:02d}:{m:02d}:{s:02d}"

    # Single-line header: name ✦ + dim metadata
    line = Text()
...
def linux_boot_sequence():
    ...
    console.print(f"  [bold white]TruthGPT[/bold white] [bold orange1]✦[/bold orange1]  [dim]v5.9-GOLD  ·  starting up  ·  {t}[/dim]")
    console.print(f"  [dim]{'─' * (w - 4)}[/dim]")
    console.print()

    for label, detail in stages:
        console.print(f"  [bold green]✔[/bold green]  [white]{label:<18}[/white]  [dim]{detail}[/dim]")
        time.sleep(0.08)

    console.print()
    console.print(f"  [dim]{'─' * (w - 4)}[/dim]")
    console.print(f"  [bold white]TruthGPT[/bold white] [bold orange1]✦[/bold orange1]  [dim]ready  ·  engine: {engine}  ·  user: {user}  ·  cost: ${budget['total_usd']:.4f}[/dim]")
    console.print(f"  [dim]{'─' * (w - 4)}[/dim]")
    console.print()
    time.sleep(0.15)
...
def _get_input_impl(message: str, choices: Optional[List[str]] = None, default: str = "", password: bool = False) -> str:
