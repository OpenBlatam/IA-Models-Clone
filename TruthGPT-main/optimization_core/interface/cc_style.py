...
def cc_tool_start(tool_name: str, input_data: str) -> None:
    """Render the start of a tool execution with its input."""
    _console.print(f"\n[bold magenta]🛠️ Executing:[/bold magenta] [cyan]{tool_name}[/cyan]")
    clean_input = str(input_data).replace('\n', ' ')
    if len(clean_input) > 150: clean_input = clean_input[:150] + "..."
    _console.print(f"  [dim]Input:[/dim] {clean_input}")

def cc_tool_end(tool_name: str, result_data: str) -> None:
    """Render the end of a tool execution with its snippet."""
    clean_res = str(result_data).replace('\n', ' ')
    if len(clean_res) > 200: clean_res = clean_res[:200] + "..."
    _console.print(f"  [dim]Result:[/dim] [green]{clean_res}[/green]\n")
def cc_tool_call(summary: str, hint: str = "ctrl+o to expand") -> None:
    """Indented tool-call status: ``  Searching for 2 patterns… (ctrl+o to expand)``."""
...
def cc_searched(patterns: int = 0, files_read: int = 0, hint: str = "ctrl+o to expand") -> None:
