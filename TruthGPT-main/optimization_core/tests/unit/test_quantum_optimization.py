    ...
    def test_quantum_genetic_algorithm(self):
        ...
        class QuantumGeneticAlgorithm:
            ...
            def quantum_selection(self, fitness_scores):
                ...
                # Shift scores to be strictly positive if any are negative/zero
                min_fit = min(fitness_scores)
                if min_fit <= 0:
                    fitness_scores = [f - min_fit + 1e-6 for f in fitness_scores]
            ...
            def get_evolution_stats(self):
                ...
                return {
                    ...
                    'fitness_improvement': self.best_fitness - self.fitness_history[0]['best_fitness']
                }
    ...
    def test_quantum_neural_network(self):
        ...
        class QuantumNeuralNetwork:
            ...
            def _quantum_layer_forward(self, input_data, weights, biases):
                ...
                input_flat = input_data.view(-1, input_dim).numpy()
                
                # Vectorized quantum matrix multiplication
                W = weights['amplitudes'] * np.exp(1j * weights['phases'])
                superposition = input_flat @ W
                
                # Add quantum bias
                bias = biases['amplitudes'] * np.exp(1j * biases['phases'])
                quantum_output = np.abs(superposition + bias)
