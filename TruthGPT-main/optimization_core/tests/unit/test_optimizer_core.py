...
class TestOptimizationMetrics(unittest.TestCase):
    ...
    def test_convergence_detection(self):
        ...
        converging_losses = [1.0, 0.8, 0.6, 0.4, 0.2, 0.005, 0.005, 0.005, 0.005, 0.005]
        
        for i, loss in enumerate(converging_losses):
            converged = detector.check_convergence(loss)
            if i == len(converging_losses) - 1:  # Should converge at the very end
        ...
        self.assertEqual(stats['total_checks'], 6)
