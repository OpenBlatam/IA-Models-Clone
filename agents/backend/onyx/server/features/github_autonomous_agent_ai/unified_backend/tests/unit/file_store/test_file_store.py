...
class TestExternalStorageFileStore:
    ...
    def test_s3_bucket_name_configuration(self) -> None:
        ...
        with patch(
            "unified_core.file_store.file_store.S3_FILE_STORE_BUCKET_NAME", "my-test-bucket"
        ):
    ...
    def test_s3_key_generation_default_prefix(self) -> None:
        ...
        with (
            patch("unified_core.file_store.file_store.S3_FILE_STORE_PREFIX", "onyx-files"),
            patch(
                "unified_core.file_store.file_store.get_current_tenant_id",
    ...
    def test_s3_key_generation_custom_prefix(self) -> None:
        ...
        with (
            patch("unified_core.file_store.file_store.S3_FILE_STORE_PREFIX", "custom-prefix"),
            patch(
                "unified_core.file_store.file_store.get_current_tenant_id",
                return_value="test-tenant",
            ),
    ...
    def test_s3_key_generation_with_different_tenant_ids(self) -> None:
        ...
        with patch("unified_core.file_store.file_store.S3_FILE_STORE_PREFIX", "onyx-files"):
            file_store = S3BackedFileStore(bucket_name="test-bucket")
    ...
    def test_s3_save_file_mock(
        ...
        with (
            patch(
                "unified_core.file_store.file_store.S3_FILE_STORE_BUCKET_NAME", "test-bucket"
            ),
            patch("unified_core.file_store.file_store.S3_FILE_STORE_PREFIX", "onyx-files"),
            patch("unified_core.file_store.file_store.S3_AWS_ACCESS_KEY_ID", "test-key"),
            patch("unified_core.file_store.file_store.S3_AWS_SECRET_ACCESS_KEY", "test-secret"),
        ):

            # Mock the database operation to avoid SQLAlchemy issues
            with patch("unified_core.db.file_record.upsert_filerecord") as mock_upsert:
