...
from unified_core.configs.constants import POSTGRES_CELERY_BEAT_APP_NAME
...
celery_app.config_from_object("unified_core.background.celery.configs.beat")
    ...
    def _generate_schedule(
        ...
        if MULTI_TENANT:
            ...
            get_cloud_tasks_to_schedule = fetch_versioned_implementation(
                "unified_core.background.celery.tasks.beat_schedule",
        ...
        get_tasks_to_schedule = fetch_versioned_implementation(
            "unified_core.background.celery.tasks.beat_schedule", "get_tasks_to_schedule"
        )
