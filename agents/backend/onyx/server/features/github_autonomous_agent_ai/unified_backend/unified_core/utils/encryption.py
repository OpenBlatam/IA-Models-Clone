from unified_core.configs.app_configs import ENCRYPTION_KEY_SECRET
...
def encrypt_string_to_bytes(intput_str: str) -> bytes:
    versioned_encryption_fn = fetch_versioned_implementation(
        "unified_core.utils.encryption", "_encrypt_string"
    )
...
def decrypt_bytes_to_string(intput_bytes: bytes) -> str:
    versioned_decryption_fn = fetch_versioned_implementation(
        "unified_core.utils.encryption", "_decrypt_bytes"
    )
