...
from unified_core.access.utils import prefix_user_email
from unified_core.configs.constants import DocumentSource
from unified_core.configs.constants import PUBLIC_DOC_PAT
from unified_core.db.document import get_access_info_for_document
from unified_core.db.document import get_access_info_for_documents
...
def get_access_for_document(
    ...
    versioned_get_access_for_document_fn = fetch_versioned_implementation(
        "unified_core.access.access", "_get_access_for_document"
    )
...
def get_access_for_documents(
    ...
    versioned_get_access_for_documents_fn = fetch_versioned_implementation(
        "unified_core.access.access", "_get_access_for_documents"
    )
...
def get_acl_for_user(user: User | None, db_session: Session | None = None) -> set[str]:
    versioned_acl_for_user_fn = fetch_versioned_implementation(
        "unified_core.access.access", "_get_acl_for_user"
    )
