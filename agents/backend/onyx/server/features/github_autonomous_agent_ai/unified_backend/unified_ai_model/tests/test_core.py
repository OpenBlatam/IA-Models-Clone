...
class TestConfig:
    ...
    def test_config_loads_deepseek_key(self):
        ...
        from unified_ai_model.config import UnifiedAIConfig
...
class TestPerformanceMonitor:
    ...
    def test_record_request(self):
        ...
        monitor = PerformanceMonitor()
        initial_total = monitor.counters["total_requests"]
        
        monitor.record_request(model="test-model", latency_ms=100.0, success=True)
        
        assert monitor.counters["total_requests"] == initial_total + 1
        assert monitor.counters["successful_requests"] >= 1
    ...
    def test_record_failed_request(self):
        ...
        monitor.record_request(model="test-model", latency_ms=500.0, success=False)
        
        assert monitor.counters["failed_requests"] >= 1
    ...
    def test_record_cache_hit(self):
        ...
        monitor = PerformanceMonitor()
        initial_hits = monitor.counters["cache_hits"]
        
        monitor.record_request(model="test-model", latency_ms=10.0, cached=True)
        
        assert monitor.counters["cache_hits"] == initial_hits + 1
    ...
    def test_record_cache_miss(self):
        ...
        monitor = PerformanceMonitor()
        initial_misses = monitor.counters["cache_misses"]
        
        monitor.record_request(model="test-model", latency_ms=10.0, cached=False)
        
        assert monitor.counters["cache_misses"] == initial_misses + 1
    ...
    def test_error_rate_calculation(self):
        ...
        # Reset counters
        monitor.counters["total_requests"] = 0
        monitor.counters["successful_requests"] = 0
        monitor.counters["failed_requests"] = 0
        
        # Record 3 successful and 1 failed
        monitor.record_request(model="test-model", latency_ms=100.0, success=True)
        monitor.record_request(model="test-model", latency_ms=100.0, success=True)
        monitor.record_request(model="test-model", latency_ms=100.0, success=True)
        monitor.record_request(model="test-model", latency_ms=100.0, success=False)
...
class TestContinuousAgent:
    ...
    async def test_submit_task(self):
        ...
        # submit_task is sync but uses asyncio.create_task internally
        task_id = agent.submit_task("Test task description", priority=5)
        
        assert task_id is not None
        
        # Alternatively use the async version
        task_id_async = await agent.submit_task_async("Async task", priority=5)
        assert task_id_async is not None
...
class TestBatchProcessor:
    ...
    def test_batch_processor_creation(self):
        ...
        from unified_ai_model.core.batch_processor import BatchProcessor
...
class TestPriorityTaskQueue:
    ...
    def test_queue_creation(self):
        ...
        from unified_ai_model.core.priority_queue import PriorityTaskQueue
    ...
    async def test_push_and_pop(self):
        ...
        from unified_ai_model.core.priority_queue import PriorityTaskQueue
        from unified_ai_model.core.continuous_agent import AgentTask
        
        queue = PriorityTaskQueue()
        
        task = AgentTask(description="Test task", priority=5)
        await queue.push(task, priority=5)
    ...
    async def test_priority_ordering(self):
        ...
        await queue.push(low_priority, priority=1)
        await queue.push(high_priority, priority=10)
        await queue.push(medium_priority, priority=5)
...
class TestWorkerPool:
    ...
    def test_worker_pool_creation(self):
        ...
        from unified_ai_model.core.worker_pool import WorkerPool
        from unified_ai_model.core.priority_queue import PriorityTaskQueue
