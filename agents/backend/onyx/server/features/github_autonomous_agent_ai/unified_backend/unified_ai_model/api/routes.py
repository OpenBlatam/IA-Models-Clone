...
from ..core.document_exporter import DocumentExporter, ExportFormat
...
def get_document_exporter() -> DocumentExporter:
    """Get or create the document exporter instance."""
...
async def export_to_pdf(request: PDFExportRequest):
    """
    Export documents to a professional PDF.
    ...
    try:
        ...
        # Combine all documents into a single content string
        combined_content = []
        for i, doc in enumerate(request.documents):
            doc_title = doc.title or f"Document {doc.documentNumber or i + 1}"
        ...
        filename = f"bulk_export_{timestamp}"
        
        # Export to PDF using DocumentExporter
        result = await exporter.export(
            content=full_content,
            filename=filename,
            format=ExportFormat.PDF,
            title=request.title or "Bulk Export",
            metadata={
        ...
        logger.warning(f"PDF export library not available: {e}")
        raise HTTPException(
            status_code=501,
...
async def get_available_export_formats():
    ...
    try:
        ...
        return {
            "formats": formats,
            "pdf_available": "pdf" in formats,
            "docx_available": "docx" in formats,
            "excel_available": "xlsx" in formats
        }
    except Exception as e:
