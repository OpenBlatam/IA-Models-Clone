...
from kv_cache.exceptions import CacheMemoryError
...
class BaseKVCache(nn.Module):
    ...
    def forward(
        ...
        use_cache: bool = True
    ) -> tuple[torch.Tensor, torch.Tensor, StatsDict]:
        """
    ...
    def _evict_entries(self) -> None:
        ...
        if self.config.enable_gc and stats.get("evictions", 0) % GC_INTERVAL == 0:
            self.memory_manager.collect_garbage()
    
    def _update_stats(self) -> None:
        """Update cache statistics."""
        from kv_cache.constants import MB_TO_BYTES, BYTES_PER_FLOAT32
        
        total_memory_mb = self.storage.get_total_memory_mb()
        # Convert to approximate element count
        # Use appropriate bytes per element based on dtype
        bytes_per_element = {
            torch.float32: BYTES_PER_FLOAT32,
            torch.float16: 2,  # BYTES_PER_FLOAT16
            torch.bfloat16: 2,  # BYTES_PER_BFLOAT16
        }.get(self.config.dtype, BYTES_PER_FLOAT32)
    ...
    def get_stats(self, include_history: bool = False) -> StatsDict:
        """
