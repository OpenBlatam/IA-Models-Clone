...
from kv_cache.types import ConfigDict
from kv_cache.constants import BYTES_TO_MB

logger = logging.getLogger(__name__)


def get_device_info(device: torch.device) -> ConfigDict:
    """
    ...
    if device.type == "cuda" and torch.cuda.is_available():
        try:
            ...
            info["memory_allocated_mb"] = torch.cuda.memory_allocated(device) * BYTES_TO_MB
            info["memory_reserved_mb"] = torch.cuda.memory_reserved(device) * BYTES_TO_MB
            info["max_memory_mb"] = torch.cuda.max_memory_allocated(device) * BYTES_TO_MB if hasattr(torch.cuda, 'max_memory_allocated') else None
...
def get_tensor_info(tensor: torch.Tensor) -> ConfigDict:
    """
