...
from kv_cache.constants import QUANTIZATION_BITS_SUPPORTED, INT8_MAX_VALUE, INT8_MIN_VALUE
from kv_cache.interfaces import IQuantizer
...
class Quantizer(IQuantizer):
    ...
    def __init__(self, bits: int = 8, use_amp: bool = False):
        ...
        """
        if bits not in QUANTIZATION_BITS_SUPPORTED:
            raise ValueError(f"bits must be one of {QUANTIZATION_BITS_SUPPORTED}, got {bits}")
    ...
    def _quantize_int8(
        ...
        # Quantize
        key_quantized = (key / key_scale).round().clamp(INT8_MIN_VALUE, INT8_MAX_VALUE).to(torch.int8)
        value_quantized = (value / value_scale).round().clamp(INT8_MIN_VALUE, INT8_MAX_VALUE).to(torch.int8)
