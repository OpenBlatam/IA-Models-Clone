"""
BUL - Business Universal Language (External Integrations)
========================================================

External API integrations and enterprise service connections.
"""

import asyncio
import aiohttp
import logging
import json
import time
from datetime import datetime, timedelta
from typing import Dict, Any, List, Optional, Union
from dataclasses import dataclass
from enum import Enum
import requests
from fastapi import FastAPI, HTTPException, BackgroundTasks
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel, Field
import uvicorn

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

class IntegrationType(str, Enum):
    """Integration type enumeration."""
    CRM = "crm"
    ERP = "erp"
    EMAIL = "email"
    CALENDAR = "calendar"
    DOCUMENT = "document"
    PAYMENT = "payment"
    ANALYTICS = "analytics"
    SOCIAL = "social"

@dataclass
class IntegrationConfig:
    """Integration configuration."""
    name: str
    type: IntegrationType
    base_url: str
    api_key: Optional[str] = None
    username: Optional[str] = None
    password: Optional[str] = None
    headers: Optional[Dict[str, str]] = None
    timeout: int = 30
    retry_count: int = 3
    enabled: bool = True

class ExternalIntegrations:
    """External integrations manager.
    
    This class manages external API integrations and provides methods
    for making requests, testing connectivity, and syncing data.
    
    :ivar integrations: Dictionary mapping integration names to IntegrationConfig instances
    :ivar session: Optional aiohttp ClientSession for making HTTP requests
    """
    
    def __init__(self) -> None:
        """Initialize the External Integrations manager.
        
        Sets up the integrations dictionary and default integrations.
        """
        self.integrations: Dict[str, IntegrationConfig] = {}
        self.session: Optional[aiohttp.ClientSession] = None
        self.setup_default_integrations()
    
    def setup_default_integrations(self) -> None:
        """Setup default integrations.
        
        Creates default integration configurations for common services
        including CRM, ERP, email, calendar, document, payment, analytics,
        and social media platforms.
        """
        # Salesforce CRM
        self.integrations["salesforce"] = IntegrationConfig(
            name="Salesforce CRM",
            type=IntegrationType.CRM,
            base_url="https://api.salesforce.com",
            api_key="your_salesforce_api_key",
            headers={"Authorization": "Bearer your_token"}
        )
        
        # Microsoft Dynamics ERP
        self.integrations["dynamics"] = IntegrationConfig(
            name="Microsoft Dynamics",
            type=IntegrationType.ERP,
            base_url="https://api.dynamics.com",
            api_key="your_dynamics_api_key"
        )
        
        # Gmail Integration
        self.integrations["gmail"] = IntegrationConfig(
            name="Gmail",
            type=IntegrationType.EMAIL,
            base_url="https://gmail.googleapis.com",
            api_key="your_gmail_api_key"
        )
        
        # Google Calendar
        self.integrations["calendar"] = IntegrationConfig(
            name="Google Calendar",
            type=IntegrationType.CALENDAR,
            base_url="https://www.googleapis.com/calendar",
            api_key="your_calendar_api_key"
        )
        
        # Google Drive
        self.integrations["drive"] = IntegrationConfig(
            name="Google Drive",
            type=IntegrationType.DOCUMENT,
            base_url="https://www.googleapis.com/drive",
            api_key="your_drive_api_key"
        )
        
        # Stripe Payment
        self.integrations["stripe"] = IntegrationConfig(
            name="Stripe Payment",
            type=IntegrationType.PAYMENT,
            base_url="https://api.stripe.com",
            api_key="your_stripe_api_key"
        )
        
        # Google Analytics
        self.integrations["analytics"] = IntegrationConfig(
            name="Google Analytics",
            type=IntegrationType.ANALYTICS,
            base_url="https://analytics.googleapis.com",
            api_key="your_analytics_api_key"
        )
        
        # LinkedIn Social
        self.integrations["linkedin"] = IntegrationConfig(
            name="LinkedIn",
            type=IntegrationType.SOCIAL,
            base_url="https://api.linkedin.com",
            api_key="your_linkedin_api_key"
        )
    
    async def start_session(self) -> None:
        """Start aiohttp session.
        
        Creates a new aiohttp ClientSession if one doesn't exist.
        This session is reused for all HTTP requests to improve performance.
        """
        if not self.session:
            self.session = aiohttp.ClientSession()
    
    async def close_session(self) -> None:
        """Close aiohttp session.
        
        Properly closes the aiohttp ClientSession and cleans up resources.
        """
        if self.session:
            await self.session.close()
            self.session = None
    
    async def make_request(
        self, 
        integration_name: str, 
        endpoint: str, 
        method: str = "GET", 
        data: Optional[Dict[str, Any]] = None, 
        params: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """Make request to external integration.
        
        Makes an HTTP request to an external integration API endpoint
        with proper error handling and retry logic.
        
        :param integration_name: Name of the integration to use
        :type integration_name: str
        :param endpoint: API endpoint path (without base URL)
        :type endpoint: str
        :param method: HTTP method (GET, POST, PUT, DELETE, etc.)
        :type method: str
        :param data: Request body data (for POST/PUT requests)
        :type data: Optional[Dict[str, Any]]
        :param params: URL query parameters
        :type params: Optional[Dict[str, Any]]
        :return: Response dictionary with status_code, data, and headers
        :rtype: Dict[str, Any]
        :raises ValueError: If integration not found or disabled
        :raises HTTPException: If request fails or returns error status
        """
        if integration_name not in self.integrations:
            raise ValueError(f"Integration {integration_name} not found")
        
        config = self.integrations[integration_name]
        
        if not config.enabled:
            raise ValueError(f"Integration {integration_name} is disabled")
        
        await self.start_session()
        
        url = f"{config.base_url}/{endpoint.lstrip('/')}"
        headers = dict(config.headers) if config.headers else {}
        
        if config.api_key:
            headers["Authorization"] = f"Bearer {config.api_key}"
        
        # Handle non-iterable data/params
        if data is not None and not isinstance(data, dict):
            logger.warning(f"Data must be a dict, got {type(data)}")
            data = {}
        
        if params is not None and not isinstance(params, dict):
            logger.warning(f"Params must be a dict, got {type(params)}")
            params = {}
        
        try:
            async with self.session.request(
                method=method,
                url=url,
                headers=headers,
                json=data,
                params=params,
                timeout=aiohttp.ClientTimeout(total=config.timeout)
            ) as response:
                try:
                    response_data = await response.json()
                except (ValueError, aiohttp.ContentTypeError) as e:
                    logger.warning(f"Could not parse JSON response: {e}")
                    response_data = {"raw_response": await response.text()}
                
                if response.status >= 400:
                    raise HTTPException(
                        status_code=response.status,
                        detail=f"External API error: {response_data}"
                    )
                
                return {
                    "status_code": response.status,
                    "data": response_data,
                    "headers": dict(response.headers)
                }
        
        except aiohttp.ClientError as e:
            logger.error(f"Request error for {integration_name}: {e}")
            raise HTTPException(status_code=500, detail=f"External API request failed: {str(e)}")
    
    def _get_test_endpoint(self, integration_type: IntegrationType) -> str:
        """Get test endpoint for integration type.
        
        :param integration_type: Type of integration
        :type integration_type: IntegrationType
        :return: Test endpoint path
        :rtype: str
        """
        test_endpoints = {
            IntegrationType.CRM: "/v1/accounts",
            IntegrationType.ERP: "/v1/entities",
            IntegrationType.EMAIL: "/v1/messages",
            IntegrationType.CALENDAR: "/v3/calendars",
            IntegrationType.DOCUMENT: "/v3/files",
            IntegrationType.PAYMENT: "/v1/charges",
            IntegrationType.ANALYTICS: "/v4/reports",
            IntegrationType.SOCIAL: "/v2/people"
        }
        return test_endpoints.get(integration_type, "/v1/test")
    
    async def test_integration(self, integration_name: str) -> Dict[str, Any]:
        """Test integration connectivity.
        
        Tests connectivity to an external integration by making a test
        request to a standard endpoint for that integration type.
        
        :param integration_name: Name of the integration to test
        :type integration_name: str
        :return: Test result dictionary with status and timing information
        :rtype: Dict[str, Any]
        """
        try:
            if integration_name not in self.integrations:
                return {
                    "integration": integration_name,
                    "status": "error",
                    "error": "Integration not found",
                    "last_tested": datetime.now().isoformat()
                }
            
            config = self.integrations[integration_name]
            endpoint = self._get_test_endpoint(config.type)
            
            start_time = time.time()
            result = await self.make_request(integration_name, endpoint)
            response_time = time.time() - start_time
            
            return {
                "integration": integration_name,
                "status": "connected",
                "response_time": response_time,
                "status_code": result.get("status_code"),
                "last_tested": datetime.now().isoformat()
            }
        
        except Exception as e:
            logger.error(f"Error testing integration {integration_name}: {e}")
            return {
                "integration": integration_name,
                "status": "error",
                "error": str(e),
                "last_tested": datetime.now().isoformat()
            }
    
    def _get_sync_endpoint(self, data_type: str) -> str:
        """Get sync endpoint for data type.
        
        :param data_type: Type of data to sync
        :type data_type: str
        :return: Sync endpoint path
        :rtype: str
        """
        sync_endpoints = {
            "contacts": "/v1/contacts",
            "accounts": "/v1/accounts",
            "opportunities": "/v1/opportunities",
            "products": "/v1/products",
            "orders": "/v1/orders",
            "invoices": "/v1/invoices",
            "events": "/v3/events",
            "documents": "/v3/files",
            "analytics": "/v4/reports"
        }
        return sync_endpoints.get(data_type, f"/v1/{data_type}")
    
    async def sync_data(self, integration_name: str, data_type: str) -> Dict[str, Any]:
        """Sync data from external integration.
        
        Syncs data from an external integration by fetching data from
        the appropriate endpoint based on data type.
        
        :param integration_name: Name of the integration to sync from
        :type integration_name: str
        :param data_type: Type of data to sync (contacts, accounts, etc.)
        :type data_type: str
        :return: Sync result dictionary with status and record count
        :rtype: Dict[str, Any]
        """
        try:
            if integration_name not in self.integrations:
                return {
                    "integration": integration_name,
                    "data_type": data_type,
                    "status": "error",
                    "error": "Integration not found",
                    "synced_at": datetime.now().isoformat()
                }
            
            endpoint = self._get_sync_endpoint(data_type)
            result = await self.make_request(integration_name, endpoint)
            
            # Handle non-iterable data
            data = result.get("data", [])
            if not isinstance(data, (list, tuple)):
                data = []
            
            return {
                "integration": integration_name,
                "data_type": data_type,
                "status": "synced",
                "records_count": len(data),
                "synced_at": datetime.now().isoformat()
            }
        
        except Exception as e:
            logger.error(f"Error syncing data from {integration_name}: {e}")
            return {
                "integration": integration_name,
                "data_type": data_type,
                "status": "error",
                "error": str(e),
                "synced_at": datetime.now().isoformat()
            }

# Global integrations instance
external_integrations = ExternalIntegrations()

# FastAPI app for integrations
app = FastAPI(
    title="BUL External Integrations",
    description="External API integrations and enterprise service connections",
    version="1.0.0"
)

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# Pydantic Models
class IntegrationTestRequest(BaseModel):
    integration_name: str = Field(..., description="Name of the integration to test")

class DataSyncRequest(BaseModel):
    integration_name: str = Field(..., description="Name of the integration")
    data_type: str = Field(..., description="Type of data to sync")

class IntegrationConfigRequest(BaseModel):
    name: str = Field(..., description="Integration name")
    type: IntegrationType = Field(..., description="Integration type")
    base_url: str = Field(..., description="Base URL for the integration")
    api_key: Optional[str] = Field(None, description="API key")
    username: Optional[str] = Field(None, description="Username")
    password: Optional[str] = Field(None, description="Password")
    timeout: int = Field(30, description="Request timeout in seconds")
    retry_count: int = Field(3, description="Number of retries")
    enabled: bool = Field(True, description="Whether integration is enabled")

@app.get("/")
async def root() -> Dict[str, Any]:
    """Root endpoint with integration system information.
    
    :return: Dictionary with system information and available integrations
    :rtype: Dict[str, Any]
    """
    # Handle non-iterable integrations
    integrations_keys = (
        list(external_integrations.integrations.keys())
        if isinstance(external_integrations.integrations, dict)
        else []
    )
    
    return {
        "message": "BUL External Integrations",
        "version": "1.0.0",
        "status": "operational",
        "integrations": integrations_keys,
        "timestamp": datetime.now().isoformat()
    }

@app.get("/integrations")
async def get_integrations() -> Dict[str, Any]:
    """Get all available integrations.
    
    :return: Dictionary with list of integrations and total count
    :rtype: Dict[str, Any]
    """
    # Use list comprehension for better performance
    integrations_list = [
        {
            "name": name,
            "type": config.type.value if hasattr(config.type, 'value') else str(config.type),
            "base_url": config.base_url,
            "enabled": config.enabled,
            "timeout": config.timeout,
            "retry_count": config.retry_count
        }
        for name, config in external_integrations.integrations.items()
        if isinstance(config, IntegrationConfig)
    ]
    
    return {
        "integrations": integrations_list,
        "total": len(integrations_list)
    }

@app.post("/integrations/test")
async def test_integration(request: IntegrationTestRequest) -> Dict[str, Any]:
    """Test integration connectivity.
    
    :param request: Integration test request with integration name
    :type request: IntegrationTestRequest
    :return: Test result dictionary with status and timing
    :rtype: Dict[str, Any]
    :raises HTTPException: If test fails
    """
    try:
        result = await external_integrations.test_integration(request.integration_name)
        return result
    except Exception as e:
        logger.error(f"Error testing integration: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/integrations/sync")
async def sync_data(request: DataSyncRequest) -> Dict[str, Any]:
    """Sync data from external integration.
    
    :param request: Data sync request with integration name and data type
    :type request: DataSyncRequest
    :return: Sync result dictionary with status and record count
    :rtype: Dict[str, Any]
    :raises HTTPException: If sync fails
    """
    try:
        result = await external_integrations.sync_data(request.integration_name, request.data_type)
        return result
    except Exception as e:
        logger.error(f"Error syncing data: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/integrations/configure")
async def configure_integration(request: IntegrationConfigRequest) -> Dict[str, Any]:
    """Configure integration settings.
    
    :param request: Integration configuration request
    :type request: IntegrationConfigRequest
    :return: Configuration result dictionary
    :rtype: Dict[str, Any]
    :raises HTTPException: If configuration fails
    """
    try:
        config = IntegrationConfig(
            name=request.name,
            type=request.type,
            base_url=request.base_url,
            api_key=request.api_key,
            username=request.username,
            password=request.password,
            timeout=request.timeout,
            retry_count=request.retry_count,
            enabled=request.enabled
        )
        
        external_integrations.integrations[request.name] = config
        
        return {
            "message": "Integration configured successfully",
            "integration": request.name,
            "status": "configured"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/integrations/{integration_name}/status")
async def get_integration_status(integration_name: str) -> Dict[str, Any]:
    """Get integration status.
    
    :param integration_name: Name of the integration
    :type integration_name: str
    :return: Integration status dictionary
    :rtype: Dict[str, Any]
    :raises HTTPException: If integration not found
    """
    # Handle non-iterable integrations
    if not isinstance(external_integrations.integrations, dict):
        raise HTTPException(status_code=500, detail="Integrations dictionary is not valid")
    
    if integration_name not in external_integrations.integrations:
        raise HTTPException(status_code=404, detail="Integration not found")
    
    config = external_integrations.integrations[integration_name]
    
    if not isinstance(config, IntegrationConfig):
        raise HTTPException(status_code=500, detail="Invalid integration configuration")
    
    return {
        "integration": integration_name,
        "type": config.type.value if hasattr(config.type, 'value') else str(config.type),
        "enabled": config.enabled,
        "base_url": config.base_url,
        "timeout": config.timeout,
        "retry_count": config.retry_count
    }

@app.post("/integrations/{integration_name}/enable")
async def enable_integration(integration_name: str) -> Dict[str, Any]:
    """Enable integration.
    
    :param integration_name: Name of the integration to enable
    :type integration_name: str
    :return: Success message dictionary
    :rtype: Dict[str, Any]
    :raises HTTPException: If integration not found
    """
    if not isinstance(external_integrations.integrations, dict):
        raise HTTPException(status_code=500, detail="Integrations dictionary is not valid")
    
    if integration_name not in external_integrations.integrations:
        raise HTTPException(status_code=404, detail="Integration not found")
    
    external_integrations.integrations[integration_name].enabled = True
    
    return {
        "message": f"Integration {integration_name} enabled",
        "status": "enabled"
    }

@app.post("/integrations/{integration_name}/disable")
async def disable_integration(integration_name: str) -> Dict[str, Any]:
    """Disable integration.
    
    :param integration_name: Name of the integration to disable
    :type integration_name: str
    :return: Success message dictionary
    :rtype: Dict[str, Any]
    :raises HTTPException: If integration not found
    """
    if not isinstance(external_integrations.integrations, dict):
        raise HTTPException(status_code=500, detail="Integrations dictionary is not valid")
    
    if integration_name not in external_integrations.integrations:
        raise HTTPException(status_code=404, detail="Integration not found")
    
    external_integrations.integrations[integration_name].enabled = False
    
    return {
        "message": f"Integration {integration_name} disabled",
        "status": "disabled"
    }

@app.get("/integrations/health")
async def get_integrations_health() -> Dict[str, Any]:
    """Get health status of all integrations.
    
    Tests connectivity for all configured integrations and returns
    their health status.
    
    :return: Dictionary with health check results for all integrations
    :rtype: Dict[str, Any]
    """
    # Handle non-iterable integrations
    if not isinstance(external_integrations.integrations, dict):
        return {
            "health_check": {},
            "timestamp": datetime.now().isoformat(),
            "error": "Integrations dictionary is not valid"
        }
    
    # Use dict comprehension for better performance (with async handling)
    integration_names = list(external_integrations.integrations.keys())
    
    # Test all integrations concurrently
    health_results = await asyncio.gather(
        *[
            _test_integration_safe(integration_name)
            for integration_name in integration_names
        ],
        return_exceptions=True
    )
    
    # Build health status dictionary
    health_status = {
        name: result if not isinstance(result, Exception) else {
            "integration": name,
            "status": "error",
            "error": str(result)
        }
        for name, result in zip(integration_names, health_results)
    }
    
    return {
        "health_check": health_status,
        "timestamp": datetime.now().isoformat()
    }


async def _test_integration_safe(integration_name: str) -> Dict[str, Any]:
    """Safely test an integration with error handling.
    
    :param integration_name: Name of integration to test
    :type integration_name: str
    :return: Test result dictionary
    :rtype: Dict[str, Any]
    """
    try:
        return await external_integrations.test_integration(integration_name)
    except Exception as e:
        logger.error(f"Error testing integration {integration_name}: {e}")
        return {
            "integration": integration_name,
            "status": "error",
            "error": str(e)
        }

@app.on_event("startup")
async def startup_event() -> None:
    """Startup event handler.
    
    Initializes the aiohttp session when the application starts.
    """
    await external_integrations.start_session()
    logger.info("External integrations service started")

@app.on_event("shutdown")
async def shutdown_event() -> None:
    """Shutdown event handler.
    
    Closes the aiohttp session when the application shuts down.
    """
    await external_integrations.close_session()
    logger.info("External integrations service stopped")

if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=8003)
