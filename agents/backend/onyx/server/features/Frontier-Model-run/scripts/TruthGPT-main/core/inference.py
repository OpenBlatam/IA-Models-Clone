...
class InferenceEngine:
    ...
    def _sample_generate(self, 
        ...
        for _ in range(max_length - input_ids.size(1)):
            ...
            with torch.no_grad():
                ...
                # Handle both 2D (batch, vocab_size) and 3D (batch, seq_len, vocab_size) outputs
                if outputs.dim() == 3:
                    logits = outputs[:, -1, :] / temperature
                elif outputs.dim() == 2:
                    # If 2D, use the last output directly
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_generation_time': np.mean(self.inference_times) if self.inference_times else 0.0,
            'tokens_per_second': self.tokens_generated / sum(self.inference_times) if sum(self.inference_times) > 0 else 0.0,
            'cache_hit_rate': self.cache_hits / total_calls if total_calls > 0 else 0.0,
