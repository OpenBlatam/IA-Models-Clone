...
class TrainingManager:
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        if self.model is None:
            raise ValueError("Model not set. Call setup_training() first.")
        
        if self.train_loader is None:
            raise ValueError("Train data loader not set. Call setup_training() first.")
    ...
    def train(self, save_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        for epoch in range(self.config.epochs):
            ...
            # Check if loss improved (considering min_delta for early stopping)
            ...
            if improvement > self.config.min_delta:
                ...
                if self.config.save_best and save_path:
        ...
        # Ensure best_loss is a real value (not infinity)
        if self.best_loss == float('inf') and self.metrics.get('train_loss'):
            # Use the last training loss if best_loss was never updated
            self.best_loss = self.metrics['train_loss'][-1] if self.metrics['train_loss'] else 0.0
    ...
    def load_checkpoint(self, checkpoint_path: str) -> None:
        ...
        if self.model is None:
            raise ValueError("Model not set. Call setup_training() first before loading checkpoint.")
        
        if self.optimizer is None:
            raise ValueError("Optimizer not set. Call setup_training() first before loading checkpoint.")
