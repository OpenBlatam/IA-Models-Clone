"""
Flux2 Core Architecture
=======================

Adapted from the official Flux2 model implementation:
https://github.com/black-forest-labs/flux2/blob/main/src/flux2/model.py

Adapted for clothing change operations with inpainting support.
"""
...
class Flux2Core(nn.Module):
    """
    Core Flux2 model architecture adapted from official implementation.
    
    Reference: https://github.com/black-forest-labs/flux2/blob/main/src/flux2/model.py
    """
    ...
    def forward(
        ...
        """
        Forward pass through Flux2 core.
        
        Args:
            x: Image tokens [B, L_img, in_channels]
            x_ids: Image position IDs [B, L_img, 4]
            timesteps: Timestep embeddings [B]
            ctx: Text context [B, L_txt, context_in_dim]
            ctx_ids: Text position IDs [B, L_txt, 4]
            guidance: Guidance scale [B]
            
        Returns:
            Output image tokens [B, L_img, out_channels]
        """
...
class SelfAttention(nn.Module):
    """Self-attention module with QKNorm."""
...
class SiLUActivation(nn.Module):
    """SiLU activation with gated mechanism."""
...
class DoubleStreamBlock(nn.Module):
    """Double stream transformer block for image-text interaction."""
...
class MLPEmbedder(nn.Module):
    """MLP embedder for timestep and guidance."""
...
class EmbedND(nn.Module):
    """N-dimensional positional embedding with RoPE."""
