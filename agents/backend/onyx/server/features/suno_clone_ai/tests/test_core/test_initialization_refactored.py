"""
Tests refactorizados para system initialization
Usando clases base y helpers para eliminar duplicación
"""

import pytest
from unittest.mock import Mock, AsyncMock, patch
from datetime import datetime

from core.initialization import SystemInitializer
from test_helpers import BaseServiceTestCase, StandardTestMixin


class TestSystemInitializerRefactored(BaseServiceTestCase, StandardTestMixin):
    """Tests refactorizados para SystemInitializer"""
    
    @pytest.fixture
    def initializer(self):
        """Fixture para SystemInitializer"""
        return SystemInitializer()
    
    @pytest.fixture
    def mock_dependencies(self):
        """Fixture con todos los mocks necesarios"""
        with patch('core.initialization.get_container') as mock_get_container, \
             patch('core.initialization.StorageFactory') as mock_storage_factory, \
             patch('core.initialization.CacheFactory') as mock_cache_factory, \
             patch('core.initialization.MusicGeneratorFactory') as mock_music_factory, \
             patch('core.initialization.get_event_bus') as mock_get_events, \
             patch('core.initialization.get_plugin_manager') as mock_get_plugins, \
             patch('core.initialization.get_module_registry') as mock_get_registry:
            
            mock_container = Mock()
            mock_get_container.return_value = mock_container
            
            mock_storage = Mock()
            mock_storage_factory.create_storage.return_value = mock_storage
            
            mock_cache = Mock()
            mock_cache_factory.create_cache.return_value = mock_cache
            
            mock_generator = Mock()
            mock_music_factory.create_generator.return_value = mock_generator
            
            mock_event_bus = Mock()
            mock_get_events.return_value = mock_event_bus
            
            mock_plugin_manager = Mock()
            mock_get_plugins.return_value = mock_plugin_manager
            
            mock_registry = Mock()
            mock_get_registry.return_value = mock_registry
            
            yield {
                'container': mock_container,
                'storage': mock_storage,
                'cache': mock_cache,
                'generator': mock_generator,
                'event_bus': mock_event_bus,
                'plugins': mock_plugin_manager,
                'registry': mock_registry
            }
    
    def test_initializer_init(self, initializer):
        """Test de inicialización"""
        assert initializer.initialized is False
        assert initializer.initialization_order == []
        assert initializer.initialization_times == {}
    
    @pytest.mark.asyncio
    async def test_initialize_all_success(self, initializer, mock_dependencies):
        """Test de inicialización exitosa de todos los componentes"""
        results = await initializer.initialize_all()
        
        assert initializer.initialized is True
        assert len(results) > 0
        assert "dependency_container" in results
        assert "storage" in results
        assert "cache" in results
        assert "music_generator" in results
    
    @pytest.mark.asyncio
    async def test_initialize_all_already_initialized(self, initializer):
        """Test de inicialización cuando ya está inicializado"""
        initializer.initialized = True
        
        results = await initializer.initialize_all()
        
        assert results == {}
    
    @pytest.mark.asyncio
    @pytest.mark.parametrize("storage_type,cache_type,generator_type", [
        ("s3", "redis", "advanced"),
        ("local", "memory", "default"),
        ("gcs", "memcached", "ultra")
    ])
    async def test_initialize_all_with_config(self, initializer, mock_dependencies, 
                                             storage_type, cache_type, generator_type):
        """Test de inicialización con diferentes configuraciones"""
        config = {
            "storage_type": storage_type,
            "cache_type": cache_type,
            "generator_type": generator_type
        }
        
        results = await initializer.initialize_all(config=config)
        
        assert initializer.initialized is True
        from core.initialization import StorageFactory, CacheFactory, MusicGeneratorFactory
        
        StorageFactory.create_storage.assert_called_with(
            storage_type=storage_type,
            base_path="storage"
        )
        CacheFactory.create_cache.assert_called_with(cache_type=cache_type)
        MusicGeneratorFactory.create_generator.assert_called_with(generator_type=generator_type)
    
    def test_get_initialization_order(self, initializer):
        """Test de obtención del orden de inicialización"""
        initializer.initialization_order = ["service1", "service2", "service3"]
        
        order = initializer.get_initialization_order()
        
        assert order == ["service1", "service2", "service3"]
    
    def test_get_initialization_times(self, initializer):
        """Test de obtención de tiempos de inicialización"""
        initializer.initialization_times = {
            "service1": 0.1,
            "service2": 0.2
        }
        
        times = initializer.get_initialization_times()
        
        assert times["service1"] == 0.1
        assert times["service2"] == 0.2



