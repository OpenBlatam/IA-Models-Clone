...
class CollaborationService(BaseService):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self._init_redis(decode_responses=True, redis_db=0)
        self._init_database()
    ...
    def db_session(self):
        ...
        if self.db_manager:
            return self.db_manager.get_scoped_session()
        return None
    ...
    async def create_session(self, project_id: str, session_name: str, 
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error saving collaboration session to database: {e}", exc_info=True)
                if self.db_session:
                ...
                if self.db_session:
                    ...
                    if db_session:
                        ...
                        try:
                            ...
                            logger.error(f"Error updating session participants: {e}", exc_info=True)
                            self.db_session.rollback()
                            raise
        ...
        try:
            ...
            for user_id in session.participants:
                ...
                participants.append({
                    'user_id': user_id,
                    'is_online': is_online,
                    'last_seen': datetime.now(timezone.utc).isoformat()  # Would be actual last seen
                })
    ...
    async def handle_websocket_connection(self, websocket: WebSocket, session_id: str):
        ...
        try:
            ...
            handlers = {
                ...
                'content_edit': self._handle_content_edit,
    ...
    async def _handle_ping(self, connection: ActiveConnection, data: Dict[str, Any]):
        """Handle ping message"""
    ...
    async def _save_collaboration_message(self, session_id: str, user_id: str, 
        ...
        try:
            ...
            logger.error(f"Error saving collaboration message: {e}", exc_info=True)
            if self.db_session:
                self.db_session.rollback()
    ...
    async def close(self):
        ...
        try:
            if self.redis_client:
                ...
                logger.info("Redis connection closed for collaboration service")
            if self.db_session:
                self.db_session.close()
                logger.info("Database connection closed for collaboration service")
        except Exception as e:
            logger.error(f"Error closing collaboration service: {e}")
