"""
Unified Color Grading Agent
============================

Consolidated agent combining best features from both implementations.
"""
...
class UnifiedColorGradingAgent:
    """
    Unified color grading agent.
    
    Combines:
    - Service groups for organized access
    - Service accessor for unified access
    - Clean architecture
    ...
    def __init__(
        ...
        """
        Initialize unified color grading agent.
        ...
        self.output_dirs = PathUtilities.create_output_structure(
            self.output_dir,
        ...
        self.service_manager = ServiceManager(self.config, self.output_dirs)
        self.services = self.service_manager.initialize_all()
        
        # Keep factory reference for backward compatibility
        self.service_factory = self.service_manager.factory
        
        # Service groups (organized access)
        self.groups = ServiceGroups(self.services)
        
        # Service accessor (unified access)
        self.service_accessor = ServiceAccessor(self.services)
        ...
        logger.info(f"Initialized UnifiedColorGradingAgent with {max_parallel_tasks} max parallel tasks")
    ...
    async def close(self):
        ...
        logger.info("UnifiedColorGradingAgent closed")
