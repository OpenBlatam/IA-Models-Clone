...
def pytest_html_results_table_header(cells):
    ...
    try:
        import pytest_html
        cells.insert(1, pytest_html.html.th("Description"))
        cells.insert(2, pytest_html.html.th("Duration"))
    except ImportError:
        pass
...
def pytest_html_results_table_row(report, cells):
    ...
    try:
        import pytest_html
        cells.insert(1, pytest_html.html.td(report.description))
        cells.insert(2, pytest_html.html.td(report.duration))
    except ImportError:
        pass
...
def invalid_filenames():
    ...
    return [
        "",  # Empty
        "test<>file.pdf",  # Invalid characters
        "../etc/passwd",  # Path traversal
        "..\\windows\\system32",  # Windows path traversal
        "test" + "a" * 300 + ".pdf",  # Too long
        "test.pdf\x00",  # Null byte
        "test.pdf\n",  # Newline
        "test.pdf\r",  # Carriage return
        "test.pdf\t",  # Tab
    ]
...
@pytest.fixture
def valid_uuids():
    """List of valid UUIDs for testing."""
    return [
...
@pytest.fixture
def invalid_uuids():
    """List of invalid UUIDs for testing."""
    return [
        "not-a-uuid",
...
@pytest.fixture
def sample_variant_options():
    """Sample variant options for testing."""
    return {
        "max_length": 500,
        "style": "academic",
        "include_images": True,
