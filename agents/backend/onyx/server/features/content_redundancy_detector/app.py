...
# Routers (Legacy - kept for backward compatibility)
# Note: Modular routes from api.routes are preferred and registered first
try:
...
def create_app() -> FastAPI:
    ...
    # Include modular API routes (prioritized)
    try:
        from api.routes import api_router
        app.include_router(api_router, prefix="/api/v1", tags=["API Modular"])
        print("✅ Modular routers registered successfully")
    except ImportError as e:
        print(f"Warning: Modular routers not available: {e}, trying register_routers...")
        try:
            from api.routes import register_routers
            register_routers(app)
            print("✅ Modular routers registered via register_routers")
        except ImportError:
            print("Warning: Both api_router and register_routers not available, falling back to legacy router")
            # Fallback to legacy router
            app.include_router(router, prefix="/api/v1", tags=["API"])
            app.include_router(router, prefix="", tags=["Root"])
    
    # Legacy router (kept for backward compatibility, but modular routes take precedence)
    # Note: Modular routes are registered first, so they take precedence over legacy routes
    try:
        from routers import router as legacy_router
        app.include_router(legacy_router, prefix="/api/v1/legacy", tags=["Legacy API"])
        print("Legacy router registered at /api/v1/legacy for backward compatibility")
    except ImportError:
        pass
