"""
Task Execution Utilities for ParallelExecutor
==============================================
...
class TaskExecutor:
    ...
    Refactored with:
    - ExecutionContext for state encapsulation
    - ExecutionResult for typed outcomes
    - ExecutionStrategy for different execution modes
    - ExecutionHooks for extensibility
    """
    ...
    async def execute_task(
        ...
        Args:
            ...
            task_queue: Task queue for marking done
            stats: Statistics dictionary
            lock: Lock for thread-safe stats updates
            future: Optional future to resolve/reject
    ...
    async def execute_with_context(
        ...
        """
        Execute task using context object.
        ...
        exec_strategy = strategy or TaskExecutor._strategy
        ...
        try:
            result = await exec_strategy.execute(context)
            ...
            return ExecutionResult.from_error(e, context)


# Convenience function for creating retry executor
def create_retry_executor(max_retries: int = 3, delay: float = 1.0) -> TaskExecutor:
    """Create executor with retry strategy."""
    TaskExecutor.set_strategy(RetryExecutionStrategy(max_retries, delay))
    return TaskExecutor
