"""
Robust Helpers for Document Analyzer
====================================

Advanced robustness utilities for document analysis with circuit breakers, retry logic, and rate limiting.
"""
...
class AdvancedRateLimiter:
    ...
    def __init__(self, rate: float, capacity: float):
        ...
        self.lock = asyncio.Lock()
        logger.info(f"Advanced Rate Limiter initialized. Rate: {rate} req/s, Capacity: {capacity} burst.")
...
def validate_document_input(
    ...
    """Validates document input parameters."""
    if not document_path and not document_content:
        return False, "Either document_path or document_content must be provided"
    
    if document_path and not Path(document_path).exists():
        return False, f"Document path does not exist: {document_path}"
    
    if document_content and len(document_content.strip()) == 0:
        return False, "Document content cannot be empty"
    
    if document_type:
        valid_types = ["pdf", "docx", "txt", "html", "markdown", "json", "xml", "csv", "image"]
        if document_type.lower() not in valid_types:
            return False, f"Invalid document type: {document_type}. Valid types: {valid_types}"
...
# Global instances
circuit_breaker = CircuitBreaker(name="document_analyzer", failure_threshold=5, recovery_timeout=60.0)
rate_limiter = AdvancedRateLimiter(rate=10.0, capacity=50.0)
