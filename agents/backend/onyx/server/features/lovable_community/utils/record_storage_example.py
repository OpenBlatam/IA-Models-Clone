"""
Record Storage - Before/After Comparison Example

This file demonstrates the problematic code (BEFORE) and the refactored code (AFTER)
to clearly show the improvements made.
"""
...
class RecordStorage_BEFORE:
    ...
    1. ❌ No usa context managers - archivos pueden no cerrarse correctamente
    2. ❌ Errores de indentación en read() y update()
    3. ❌ Manejo incorrecto de registros en update() - reemplaza en lugar de fusionar
    4. ❌ Sin validación de entrada de usuario
    ...
    def update(self, record_id, updates):
        ...
        for record in records:
            if record['id'] == record_id:
                ...
                record = updates  # Esto reemplaza el registro completo
                break
        ...
        try:
            ...
            for i, record in enumerate(records):
                ...
                if record.get('id') == record_id:
                    # ✅ FIXED: Fusiona actualizaciones en lugar de reemplazar
                    original_id = record.get('id')
                    records[i].update(updates)  # Fusiona, no reemplaza
            ...
            # ✅ FIXED: Indentación correcta - write después del loop
            self.write(records)
            return True
...
6. PRESERVACIÓN DE ID:
   ❌ ANTES: Puede perder el ID original
   ✅ DESPUÉS: Preserva el ID original del registro
"""
