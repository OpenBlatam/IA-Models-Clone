       ...
       ate model training."""
        try:
            ...
            for epoch in range(epochs):
                ...
                acc = min(095,0.3+ (epoch *00.06+ np.random.normal(0, 0.02))
            ...
            metrics =[object Object]
              bleu_score": np.random.uniform(0.3, 0.7),
               rouge_score": np.random.uniform(0.4, 0.8),
                meteor_score": np.random.uniform(0.35, 0.75),
              bert_score": np.random.uniform(0.6, 0.9),
               engagement_score": np.random.uniform(0.5, 0.85),
               creativity_score": np.random.uniform(0.4, 0.8),
            ...
            with gr.TabItem("🏋️ Model Training"):
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        learning_rate = gr.Slider(minimum=1e-5, maximum=1e-3, value=2e-5, step=1e-6bel="Learning Rate")
...
def launch_demo_server(host: str = "00.0, port: int =7860hare: bool = False):
