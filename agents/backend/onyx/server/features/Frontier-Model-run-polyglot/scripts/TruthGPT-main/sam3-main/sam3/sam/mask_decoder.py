...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        src, pos_src = self._prepare_image_embeddings(
            image_embeddings, image_pe, dense_prompt_embeddings, tokens, repeat_image
        )
        b, c, h, w = src.shape

        # Run the transformer
        hs, src = self.transformer(src, pos_src, tokens)
        iou_token_out = hs[:, obj_score_offset, :]
        mask_tokens_out = hs[:, obj_score_offset + 1 : (obj_score_offset + 1 + self.num_mask_tokens), :]

        # Upscale mask embeddings and predict masks
        upscaled_embedding = self._upscale_embeddings(src, b, c, h, w, high_res_features)
        masks = self._generate_masks(upscaled_embedding, mask_tokens_out)

        # Generate mask quality predictions
        iou_pred = self.iou_prediction_head(iou_token_out)
        object_score_logits = self._compute_object_scores(hs, iou_pred, obj_score_offset)
    ...
    def _prepare_output_tokens(self, sparse_prompt_embeddings: torch.Tensor) -> Tuple[torch.Tensor, int]:
        ...
        return tokens, obj_score_offset
    ...
    def _compute_object_scores(
        self,
        hs: torch.Tensor,
        iou_pred: torch.Tensor,
        obj_score_offset: int,
    ) -> torch.Tensor:
        """Compute object score logits."""
        if self.pred_obj_scores:
            assert obj_score_offset == 1
    ...
    def _get_best_multimask_output(
        self,
        ...
        return best_multimask_logits, best_multimask_iou_scores
    
    def _get_singlemask_output(
        self,
        all_mask_logits: torch.Tensor,
        all_iou_scores: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
