...
class OptimizationConfig:
    ...
    def __post_init__(self):
        """Set level-specific defaults and validate level"""
        # Validate level
        if not isinstance(self.level, OptimizationLevel):
            try:
                # Try to convert string to OptimizationLevel
                self.level = OptimizationLevel(self.level)
            except (ValueError, TypeError):
                raise ValueError(f"Invalid optimization level: {self.level}. Must be one of {[e.value for e in OptimizationLevel]}")
