...
class ProductionPiMoESystem(AdvancedPiMoESystem):
    ...
    def __init__(self, config: ProductionConfig):
        ...
        super().__init__(
            ...
            expert_types=expert_types[:config.num_experts],
            routing_config=advanced_config,
        )
            ...
            with torch.no_grad():
                if return_comprehensive_info:
                    output, comprehensive_info = self.forward(
                        input_tensor, attention_mask, return_advanced_info=True
                    )
                else:
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        return {
            'system_type': 'ProductionPiMoESystem',
            'hidden_size': self.hidden_size,
            'num_experts': self.num_experts,
            'model_version': self.production_config.model_version,
            'deployment_id': self.production_config.deployment_id,
            'environment': self.production_config.environment,
        }
...
def run_production_demo():
    ...
    if response['success'] and 'output' in response:
        out = response['output']
        print(f"  Output Shape: {len(out)} x {len(out[0])} x {len(out[0][0])}")
