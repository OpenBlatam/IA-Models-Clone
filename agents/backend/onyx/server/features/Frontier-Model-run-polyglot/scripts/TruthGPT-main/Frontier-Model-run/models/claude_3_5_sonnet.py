...
ClaudeRMSNorm = RMSNorm
ClaudeLinear = SafetyLinear
...
class ClaudeAttention(nn.Module):
    ...
    def forward(
        ...
        safety_context: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
    ...
    def _compute_safety_scores(
        ...
        safety_context: torch.Tensor
    ) -> torch.Tensor:
        """Compute safety modulation scores."""
...
class ClaudeFeedForward(nn.Module):
    ...
    def _apply_constitutional_constraints(self, x: torch.Tensor) -> torch.Tensor:
...
class ClaudeTransformerBlock(nn.Module):
    ...
    def _forward_impl(
        ...
        freqs_cis: torch.Tensor,
        mask: Optional[torch.Tensor],
        safety_context: Optional[torch.Tensor]
    ) -> torch.Tensor:
        if self.use_mixture_of_depths:
...
class ClaudeTransformer(nn.Module):
    ...
    def _generate_safety_context(self, embeddings: torch.Tensor) -> torch.Tensor:
    ...
    def _apply_constitutional_output(self, hidden_states: torch.Tensor) -> torch.Tensor:
