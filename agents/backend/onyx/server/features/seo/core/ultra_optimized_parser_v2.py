...
@dataclass
class ParsingStats:
    """Estadísticas de parsing ultra-optimizado."""
    parser_used: str = ""
...
class UltraOptimizedParserV2(HTMLParserInterface):
...
self.config = config or {}
        ...
        self.max_size = self.config.get('max_size', 50 * 1024 * 1024)  # 50MB
        ...
        logger.info("Ultra-Optimized Parser v2.0 initialized")
    
    def parse(self, html_content: str, url: Optional[str] = None) -> ParsedData:
        """Parse HTML ultra-optimizado con fallback automático."""
        ...
        if self.enable_compression and original_size > 1024:
            try:
                compressed_data = self._compress_data(parsed_data)
    ...
    def _parse_with_selectolax(self, html_content: str, url: Optional[str] = None) -> ParsedData:
        """Parse usando Selectolax (más rápido)."""
    ...
    def _extract_links_selectolax(self, parser: SelectolaxParser, base_url: Optional[str]) -> List[Dict[str, str]]:
        """Extraer links usando Selectolax."""
        links = []
    ...
    def _extract_images_selectolax(self, parser: SelectolaxParser, base_url: Optional[str]) -> List[Dict[str, str]]:
        """Extraer imágenes usando Selectolax."""
        images = []
    ...
    def _extract_text_selectolax(self, parser: SelectolaxParser) -> str:
        """Extraer texto usando Selectolax."""
        ...
        # Fallback: texto de todo el documento
        return parser.text()
    ...
    def _extract_text_lxml(self, tree) -> str:
        ...
        if body_text:
            return ' '.join(text.strip() for text in body_text if text.strip())
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            'parser_type': 'ultra_optimized_v2',
