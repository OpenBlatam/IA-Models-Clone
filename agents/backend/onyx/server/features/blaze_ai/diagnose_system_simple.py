...
class SystemDiagnostic:
    ...
    def collect_system_info(self):
        ...
        print(f"  [OK] Platform: {self.system_info['platform']}")
        print(f"  [OK] Python: {self.system_info['python_version'].split()[0]}")
        print(f"  [OK] Executable: {self.system_info['python_executable']}")
        print(f"  [OK] Current Dir: {self.system_info['current_directory']}")
    ...
    def check_disk_space(self):
        ...
        try:
            ...
            print(f"  [INFO] Total: {total_gb:.2f} GB")
            print(f"  [INFO] Used: {used_gb:.2f} GB")
            print(f"  [INFO] Free: {free_gb:.2f} GB")
    ...
    def check_python_installation(self):
        ...
        try:
            ...
            print(f"  [OK] Python version check: {result.stdout.strip()}")
        except Exception as e:
        ...
        for file_path in test_files:
            if os.path.exists(file_path):
                try:
                    ...
                    print(f"  [OK] {file_path} is readable")
                    ...
                    try:
                        ...
                        print(f"  [OK] {file_path} directory is writable")
                    except Exception as e:
...
try:
    ...
    print("  [OK] Plugin imports working")
except Exception as e:
    print(f"  [ERROR] Plugin imports failed: {e}")
"""
            ...
            if result.returncode == 0:
                ...
                print(f"  [ERROR] Import test failed: {result.stderr.strip()}")
    ...
    def check_network_connectivity(self):
        ...
        try:
            ...
            if result.returncode == 0:
                print("  [OK] Local network connectivity working")
            else:
                print("  [WARN] Local network connectivity issues")
                
        except Exception as e:
            print(f"  [WARN] Could not check network: {e}")
    ...
    def generate_report(self):
        ...
        if not self.issues:
            print("[SUCCESS] No issues detected! Your system appears to be working correctly.")
            return
        
        print(f"[ERROR] Found {len(self.issues)} issue(s):")
        for i, issue in enumerate(self.issues, 1):
            print(f"  {i}. {issue}")
        
        if self.suggestions:
            print(f"\n[SUGGESTION] Suggestions to resolve issues:")
...
def main():
    ...
    try:
        ...
        if success:
            print("\n[SUCCESS] System diagnostic completed successfully!")
            sys.exit(0)
        else:
            print("\n[WARNING] System diagnostic found issues that need attention.")
            sys.exit(1)
        ...
        print(f"\n[ERROR] Unexpected error during diagnostic: {e}")
        sys.exit(1)
