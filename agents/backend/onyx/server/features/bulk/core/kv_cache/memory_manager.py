...
from kv_cache.config import KVCacheConfig
from kv_cache.constants import MB_TO_BYTES, BYTES_TO_MB
from kv_cache.interfaces import IMemoryManager
...
class MemoryManager(IMemoryManager):
        ...
        if self.config.max_memory_mb is not None:
            try:
                if torch.cuda.is_available() and self.device.type == "cuda":
                    memory_allocated = torch.cuda.memory_allocated(self.device) * BYTES_TO_MB
                    memory_reserved = torch.cuda.memory_reserved(self.device) * BYTES_TO_MB
                    ...
                    # Rough estimate: cache_size * head_dim * 4 bytes (float32)
                    BYTES_PER_ELEMENT = 4
                    estimated_mb = (
    ...
    def get_memory_stats(self) -> StatsDict:
        """
        ...
        try:
            if torch.cuda.is_available() and self.device.type == "cuda":
                ...
                stats["reserved_mb"] = torch.cuda.memory_reserved(self.device) * BYTES_TO_MB
                stats["max_allocated_mb"] = torch.cuda.max_memory_allocated(self.device) * BYTES_TO_MB
                stats["max_reserved_mb"] = torch.cuda.max_memory_reserved(self.device) * BYTES_TO_MB
        except Exception as e:
